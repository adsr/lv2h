...
int main(int argc, char **argv) {
    ...
    lv2h_plug_new(host, "git://github.com/Themaister/fmsynth", &plug[0]);
    lv2h_inst_new(plug[0], &inst[0]);
    ...
    lv2h_inst_connect(inst[0], "output_left", inst[1], "in_l");
    lv2h_inst_connect(inst[0], "output_right", inst[1], "in_l");
    ...
    lv2h_node_set_count_limit(node[0], 24);
...
static int lv2h_node_callback0(lv2h_node_t *node, void *udata, int count) {
    //printf("0count=%d\n", count);
    uint8_t note;
    note = (count + 0x45) % 0x7f;
    lv2h_inst_play(inst[0], "midi", 0, note, -1, -1, -1, 0x70, 300);
    return 0;
}
...
static int lv2h_node_callback1(lv2h_node_t *node, void *udata, int count) {
    ...
    lv2h_inst_play(inst[2], "midi_in", 0, note, -1, -1, -1, 0x70, 300);
    return 0;
}
